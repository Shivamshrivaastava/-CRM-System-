...
export async function createLeadHandler(req, res) {
    ...
    for (const m of managers) {
        if (m.email) {
            sendMail(m.email, "🆕 New Lead Created", `<h3>${lead.name}</h3><p>Value: ${lead.value ?? "-"}</p><p>Source: ${lead.source ?? "-"}</p>`);
        }
    }
}
export async function updateLeadHandler(req, res) {
    const userId = req.user.sub;
    const { id } = req.params;
    ...
    if (lead.ownerId)
        io.to(`user:${lead.ownerId}`).emit("lead:updated", { lead });
    // Email: notify owner
    if (lead.ownerId) {
        const owner = await prisma.user.findUnique({ where: { id: lead.ownerId } });
        if (owner?.email) {
            sendMail(owner.email, "🔄 Lead Updated", `<p>Your lead <b>${lead.name}</b> is now <b>${lead.status}</b>.</p>`);
        }
    }
}
export async function assignOwnerHandler(req, res) {
    const { id } = req.params;
    ...
    if (user?.email) {
        sendMail(user.email, " Lead Assigned", `<p>You have been assigned a new lead: <b>${lead.name}</b>.</p>`);
    }
}
